...
- **Group B: Predictive Modeling & Optimization**
    - **Stationarity Testing**: Conducted Augmented Dickey-Fuller (ADF) tests to ensure the data was suitable for time-series modeling.
    - **Statistical Forecasting**: Designed and tuned a SARIMAX model with a seasonal order of $(0, 1, 0, 52)$ to capture annual holiday cycles.
    - **Machine Learning**: Implemented an XGBoost Regressor to capture non-linear relationships between markdowns, store size, and sales.
    - **Transformation**: Managed the complex process of scaling exogenous variables and performing inverse-log transforms for final dollar-value predictions.
    - **Metric Governance: Implemented the Weighted Mean Absolute Error (WMAE) function, ensuring holiday errors were weighted 5x heavier than regular weeks.
- **Group C: Exploratory Data Analysis & Insights**
    - **Diagnostic Visualization**: Produced Actual vs. Forecast plots to identify "Forecast Bias" and level shifts.
